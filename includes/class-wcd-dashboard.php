...
class WCD_Dashboard {
    public function __construct() {
        // Remove default dashboard content - prioridade 5 para remover antes de adicionar nosso conteúdo
        add_action('init', array($this, 'remove_default_dashboard'));
        
        // Adiciona nosso conteúdo personalizado - prioridade 10 (padrão)
    ...
    private function render_recent_orders_card() {
        ...
        echo '<h3>' . esc_html__('Encomendas Recentes', 'woo-custom-dashboard') . '</h3>';
        ...
        if ($orders) {
            ...
            foreach ($orders as $order) {
                ...
                echo '<a href="' . esc_url($order->get_view_order_url()) . '" class="wcd-view-order">' . 
                     esc_html__('Ver', 'woo-custom-dashboard') . '</a>';
                echo '</li>';
            }
            echo '</ul>';
        } else {
            echo '<p class="wcd-no-orders">' . esc_html__('Ainda não existem encomendas.', 'woo-custom-dashboard') . '</p>';
        }
    ...
    private function render_quick_stats_card() {
        ...
        echo '<h3>' . esc_html__('As Suas Estatísticas', 'woo-custom-dashboard') . '</h3>';
        ...
        echo '<span class="wcd-stat-label">' . esc_html__('Encomendas Realizadas', 'woo-custom-dashboard') . '</span>';
        echo '</div>';
        
        // Total spent
        echo '<div class="wcd-stat-item">';
        echo '<span class="wcd-stat-value">' . wp_kses_post($total_spent) . '</span>';
        echo '<span class="wcd-stat-label">' . esc_html__('Total Gasto', 'woo-custom-dashboard') . '</span>';
        echo '</div>';
    ...
    private function render_account_actions_card() {
        ...
        echo '<h3>' . esc_html__('Ações Rápidas', 'woo-custom-dashboard') . '</h3>';
        ...
        echo '<span class="wcd-action-label">' . esc_html__('Transferências', 'woo-custom-dashboard') . '</span>';
        echo '</a>';
        ...
        echo '<span class="wcd-action-label">' . esc_html__('Encomendas', 'woo-custom-dashboard') . '</span>';
        echo '</a>';
        ...
        echo '<span class="wcd-action-label">' . esc_html__('Detalhes da Conta', 'woo-custom-dashboard') . '</span>';
        echo '</a>';
        ...
        echo '<span class="wcd-action-label">' . esc_html__('Endereços', 'woo-custom-dashboard') . '</span>';
        echo '</a>';
